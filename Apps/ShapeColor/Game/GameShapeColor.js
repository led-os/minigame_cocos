...
var GameShapeColor = cc.Class({
    ...
    OnGameBomb: function () {
        ...
        cc.ViewAlertManager.main().ShowFull({
            ...
            no: no,
            isShowBtnNo: true,
            name: "STR_KEYNAME_VIEWALERT_GAME_BOMB",
    ...
    GetItemInfoShapeColor: function (idx, list) {
        if (list == null) {
            cc.Debug.Log("GetItemInfoShapeColor list = null");
            return null;
        }
        if (idx >= list.length) {
            cc.Debug.Log("GetItemInfoShapeColor out of range idx= " + idx + " list.length=" + list.length);
            return null;
        }
        var info = list[idx];
        cc.Debug.Log("GetItemInfoShapeColor  idx= " + idx + " list.length=" + list.length);
        return info;
    },
    ...
    CreateItem: function (infoshape, infocolor, isInner, isMain, i, j) {
        cc.Debug.Log("CreateItem start");
        if (this.node == null) {
            return;
        }
        cc.Debug.Log("CreateItem=" + " id=" + infoshape.id + " pic=" + infoshape.pic);
        //添加尾巴 ShapeTrail
        // if (isInner)
        // {
